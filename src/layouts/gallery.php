...
<body id = "body">
    ...
    <main class="row">
        ...
        <article class="col-6">
                <form action="/saveChoice" method="POST">
                    <div class="card formstyle">
                        <?php if ($total_pages != 0) { ?>
                            <input type="submit" value="Zapamiętaj wybrane">
                        <?php } else { ?>
                            <p>W galerii nie ma zdjęć<p>
                        <?php } ?>
                    </div>
                    <?php foreach($gallery as $photo): ?>
                        <div class="card">
                            <div class="formstyle">
                                <label for="cart[]">Dodaj do wybranych
                                    <input type="checkbox" name="cart[]" value="<?php echo $photo->_id; ?>" 
                                        <?php if (in_array($photo->_id, $_SESSION['cart'])) echo "checked='checked'";?>> 
                                </label>
                            </div>
        ...
        <nav class="col-3 right">
            <div class="card">
                <div class="tabelschoice">
                    <ol>
                        <li><a href="/gallery/chosen">Przejdź do wybranych</a></li>
                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                            <li><a href="/gallery?page=<?php echo $i; ?>">Strona <?php echo $i?></a></li>
                        <?php endfor; ?>
                    </ol>
                </div>
            </div>
